...
export const webSearch = ({ session }: WebSearchProps) =>
  tool({
    description:
      'Performs a real-time web search using the Tavily API and returns structured search results.',
    parameters: z.object({
      ...
      searchDepth: z
        .enum(['basic', 'advanced'])
        .optional()
        .describe('Depth of the search.'),
      includeAnswer: z
        .union([z.boolean(), z.literal('basic'), z.literal('advanced')])
        .optional()

...
export const streamingDocument = ({
  session,
  dataStream,
}: CreateDocumentProps) =>
  tool({
    ...
    parameters: z.object({
      title: z
        .string()
        .describe('The title or topic to generate content about.'),
      kind: z
        .enum(artifactKinds)
        .describe('The kind of content to generate (e.g., text).'),
    }),
    execute: async ({ title, kind }) => {
      ...
      if (!documentHandler) {
        throw new Error(
          `No document handler found for requested kind: ${kind}`,
        );
      }

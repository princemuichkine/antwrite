...
export function synonymsPlugin(): Plugin<SynonymPluginState> {
  ...
  function showOverlay(
    ...
    closeOverlayListener = (event: MouseEvent) => {
      if (
        overlayContainer &&
        !overlayContainer.contains(event.target as Node)
      ) {
        hideOverlay(view);
  ...
  return new Plugin<SynonymPluginState>({
    ...
    state: {
      ...
      apply(
        tr: Transaction,
        pluginState: SynonymPluginState,
        ...
        if (tr.docChanged || nextLoadingPos !== pluginState.loadingPos) {
          const nextDecorationSet = buildDecorations(
            newState.doc,
            nextLoadingPos,
          );
          return {
            decorationSet: nextDecorationSet,
            loadingPos: nextLoadingPos,
          };
        }
    ...
    props: {
      ...
      handleDOMEvents: {
        mouseover(view, event) {
          ...
          while (
            currentSentStart > paragraphStart &&
            !/[.!?]/.test(
          ...
          if (
            currentSentStart > paragraphStart + 1 &&
            /[.!?]\s/.test(
              view.state.doc.textBetween(
                currentSentStart - 2,
                currentSentStart,
          ...
          while (
            currentSentEnd < paragraphEnd &&
            !/[.!?]/.test(
              view.state.doc.textBetween(currentSentEnd, currentSentEnd + 1),
            )
          ) {
            currentSentEnd++;
          }
          ...
          if (currentSentStart > paragraphStart) {
            ...
            while (
              prevSentEnd > paragraphStart &&
              /\s/.test(
                view.state.doc.textBetween(prevSentEnd - 1, prevSentEnd),
              )
            ) {
            ...
            while (
              prevSentStart > paragraphStart &&
              !/[.!?]/.test(
                view.state.doc.textBetween(prevSentStart - 1, prevSentStart),
              )
            ) {
              prevSentStart--;
            }
          ...
          if (currentSentEnd < paragraphEnd) {
            ...
            while (
              nextSentStart < paragraphEnd &&
              /\s/.test(
                view.state.doc.textBetween(nextSentStart, nextSentStart + 1),
              )
            ) {
            ...
            while (
              nextSentEnd < paragraphEnd &&
              !/[.!?]/.test(
                view.state.doc.textBetween(nextSentEnd, nextSentEnd + 1),
              )
            ) {
              nextSentEnd++;
            }
          ...
          const context = [prevSentText, currSentText, nextSentText]
            .map((s) => s.trim())
            .filter(Boolean)
            .join(' ');
          ...
          if (
            currentState?.loadingPos?.from === from &&
            currentState.loadingPos.to === to
          ) {
            return false;
          }
          ...
          fetch(`/api/synonyms`, {
            ...
            .then((data) => {
              ...
              if (
                latestState?.loadingPos?.from === from &&
                latestState.loadingPos.to === to
              ) {

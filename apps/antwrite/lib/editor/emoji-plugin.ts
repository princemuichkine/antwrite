...
export function emojiPlugin(): Plugin {
  ...
  function insertEmojiAtCursor(emojiCode: string): boolean {
    ...
    if (from < 0 || from > doc.content.size) {
      console.warn(
        'Emoji plugin: Invalid cursor position:',
        from,
        'doc size:',
        doc.content.size,
      );
    ...
    if (start >= 0 && doc.textBetween(start, start + 1) === ':') {
      ...
      if (
        ...
        console.warn('Emoji plugin: Calculated positions out of bounds:', {
          start,
          end,
  ...
  function renderOverlay(): void {
    ...
    pluginState.overlayRoot.render(
      React.createElement(EmojiOverlay, {
        ...
        position: {
          x: pluginState.coords.left,
          y: pluginState.coords.top + 20,
        },
  ...
  function scrollToSelectedEmoji(): void {
    ...
    const selectedItem = pluginState.suggestionElement.querySelector(
      `[data-index="${pluginState.selectedIndex}"]`,
    ) as HTMLElement;
  ...
  (window as any).insertEmojiSuggestion = (emojiCode: string): void => {
    ...
    if (!success) {
      console.warn(
        'Emoji plugin: Failed to insert emoji, could not find colon',
      );
    }
              ...
              if (
                ...
                if (
                  ...
                  if (calculatedPos >= 0 && calculatedPos <= docSize) {
                    const coords =
                      pluginState.editorView.coordsAtPos(calculatedPos);
                    if (coords) {
                      showSuggestions(partialMatch[0], coords);
                    }
                  } else {
                    console.warn(
                      'Emoji plugin: Calculated position out of bounds:',
                      calculatedPos,
                      'doc size:',
                      docSize,
                    );
                  ...
                  console.warn(
                    'Emoji plugin: Invalid partialMatch.index:',
                    partialMatch.index,
                  );
                }
              } else if (
    ...
    keymap: {
      ':': (
        ...
        if (from >= 0 && from <= view.state.doc.content.size) {
          ...
          console.warn(
            'Emoji plugin: Invalid cursor position for colon key:',
            from,
          );
        }
        return false;
      ...
      ArrowRight: (): boolean => {
        if (pluginState.suggestions.length > 0) {
          pluginState.selectedIndex = Math.min(
      ...
      ArrowLeft: (): boolean => {
        if (pluginState.suggestions.length > 0) {
          pluginState.selectedIndex = Math.max(
            pluginState.selectedIndex - 1,
            0,
          );
          renderOverlay();
      ...
      Enter: (): boolean => {
        if (
          ...
          pluginState.selectedIndex < pluginState.suggestions.length
        ) {

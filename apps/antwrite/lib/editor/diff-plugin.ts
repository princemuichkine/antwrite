  ...
  return new Plugin({
    ...
    view(editorView: EditorView) {
      const handlePreviewUpdate = (event: CustomEvent) => {
        ...
        if (!previewActiveRef) {
          previewOriginalContentRef = buildContentFromDocument(
            editorView.state.doc,
          );
        }

        const oldContent =
          previewOriginalContentRef ??
          buildContentFromDocument(editorView.state.doc);
      ...
      const handleCancelPreview = (event: CustomEvent) => {
        ...
        const originalDocNode = buildDocumentFromContent(
          previewOriginalContentRef,
        );
        const tr = editorView.state.tr.replaceWith(
          0,
          editorView.state.doc.content.size,
          originalDocNode.content,
        );
        ...
        const finalizeApply = async () => {
          ...
          state.doc.descendants((node, pos) => {
            ...
            const deletedMark = node.marks.find(
              (mark) =>
                mark.type === diffMarkType &&
                mark.attrs.type === DiffType.Deleted,
            );
            if (deletedMark) {
      ...
      window.addEventListener(
        'cancel-document-update',
        handleCancelPreview as EventListener,
      );
      window.addEventListener(
        'apply-document-update',
        handleApply as EventListener,
      );

...
export function inlineSuggestionPlugin(options: {
  requestSuggestion?: (state: EditorState) => void;
  ...
  return new Plugin<InlineSuggestionState>({
      ...
      apply(
        ...
        if (metaSet) {
          ...
          if (
            ...
            pluginState.suggestionPos === newState.selection.head
          ) {
        ...
        if (
          ...
          (pluginState.isLoading || pluginState.suggestionText)
        ) {
          if (
            tr.docChanged ||
    ...
    props: {
      ...
      handleKeyDown(view: EditorView, event: KeyboardEvent): boolean {
        ...
        if (
          event.key === 'Escape' &&
          (pluginState.suggestionText || pluginState.isLoading)
        ) {
          event.preventDefault();

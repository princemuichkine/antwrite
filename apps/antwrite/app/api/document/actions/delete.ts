...
export async function deleteDocument(request: NextRequest, body: any) {
  try {
    ...
    if (!session?.user?.id) {
      console.warn(
        '[Document API - DELETE] Delete request unauthorized - no session',
      );
    ...
    console.log(
      `[Document API - DELETE] User ${userId} requested deletion for document: ${documentId}`,
    );
    ...
    if (
      ...
      console.error(
        `[Document API - DELETE] Invalid document ID for deletion: ${documentId}`,
      );
    ...
    if (!uuidRegex.test(documentId)) {
      console.error(
        `[Document API - DELETE] Invalid document ID format: ${documentId}`,
      );
      return NextResponse.json(
        {
    ...
    await deleteDocumentByIdAndUserId({
      userId: userId,
      documentId: documentId,
    });

    console.log(
      `[Document API - DELETE] Document deleted successfully: ${documentId}`,
    );
    return NextResponse.json({
      success: true,
    ...
    if (error.message?.includes('Unauthorized')) {
      ...
      message =
        'Unauthorized - you do not own this document or it does not exist';

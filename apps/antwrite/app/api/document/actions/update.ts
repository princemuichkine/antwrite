...
export async function updateDocument(
  ...
  try {
    ...
    try {
      const currentVersion = await getCurrentDocumentVersion({
        userId,
        documentId,
      });
      ...
      if (currentVersion?.updatedAt) {
        ...
        console.log(
          `[Document API - UPDATE] Time Check - Now: ${new Date().toISOString()}, UpdatedAt: ${currentVersion.updatedAt.toISOString()}`,
        ); // Log actual dates used

        if (
          minutesSinceLastUpdate < VERSION_THRESHOLD_MINUTES &&
          metadataMatches
        ) {
          shouldUpdateCurrent = true;
        }
        console.log(
          `[Document API - UPDATE] Threshold Check for ${documentId}: ` +
        ...
        const latestVersionForTitle = await getLatestDocumentById({
          id: documentId,
        });
        if (latestVersionForTitle) {
          titleForNewVersion = latestVersionForTitle.title;
          console.log(
            `[Document API - UPDATE] No current version found for ${documentId}, using title "${titleForNewVersion}" from latest version.`,
          );
        } else {
          console.log(
            `[Document API - UPDATE] No current or latest version found for ${documentId}. Creating new version with default title.`,
          );
        }
      }
      ...
      if (shouldUpdateCurrent) {
        ...
        console.log(
          `[Document API - UPDATE] Updating current version for ${documentId} (within ${VERSION_THRESHOLD_MINUTES} mins threshold and metadata matches)`,
        );
        ...
        if (!updatedOrCreatedDocument) {
          throw new Error(
            'updateCurrentDocumentVersion returned null unexpectedly.',
          );
        }
      } else {
        // --- Create New Version ---
        console.log(
          `[Document API - UPDATE] Creating new version for ${documentId} (no current, threshold exceeded, or metadata changed)`,
        );

        let finalChatId: string | null = null;
        if (inputChatId) {
          console.log(
            `[Document API - UPDATE] Checking if chat ${inputChatId} exists...`,
          );
          const chatExists = await getChatExists({ chatId: inputChatId });
          if (chatExists) {
            finalChatId = inputChatId;
            console.log(
              `[Document API - UPDATE] Verified chat exists: ${inputChatId}`,
            );
          } else {
            console.warn(
              `[Document API - UPDATE] Chat with ID ${inputChatId} not found or invalid, creating new document version without chat link`,
            );
          }
        }
        ...
        if (!updatedOrCreatedDocument) {
          throw new Error(
            'createNewDocumentVersion returned null unexpectedly.',
          );
        }
      }
      ...
      if (!finalDocumentState) {
        console.error(
          `[Document API - UPDATE] Failed to retrieve document ${documentId} after operation.`,
        );
        const fallbackState = await getLatestDocumentById({ id: documentId });
        if (fallbackState) {
          console.warn(
            '[Document API - UPDATE] Returning fallback state after initial retrieval failed.',
          );
          ...
          return NextResponse.json(
            {
              error:
                'Failed to retrieve updated document data after operation and fallback.',
            },
            { status: 500 },
          );
        }

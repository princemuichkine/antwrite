  ...
  try {
    ...
    if (limitParam) {
      try {
        ...
        const result = await getPaginatedDocumentsByUserId({
          userId,
          limit,
          endingBefore,
        });
        ...
        console.error(
          '[Document API - GET] Error fetching paginated documents:',
          error,
        );
        return NextResponse.json(
    ...
    // --- Fetch all *current* documents for the user (legacy) ---
      ...
      try {
        ...
        console.error(
          '[Document API - GET] Error fetching all current documents:',
          error,
        );
        return NextResponse.json(

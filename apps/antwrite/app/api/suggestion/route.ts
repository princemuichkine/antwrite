...
async function handleSuggestionRequest(
  ...
  (async () => {
    try {
      ...
      await writer.write(
        encoder.encode(
          `data: ${JSON.stringify({ type: 'id', content: documentId })}\n\n`,
        ),
      );
      ...
      if (isPartialEdit) {
        ...
        await writer.write(
          encoder.encode(
            `data: ${JSON.stringify({ type: 'original', content: selectedText })}\n\n`,
          ),
        );
      } else {
        ...
        await writer.write(
          encoder.encode(
            `data: ${JSON.stringify({ type: 'clear', content: '' })}\n\n`,
          ),
        );
      }
      ...
      await writer.write(
        encoder.encode(
          `data: ${JSON.stringify({ type: 'error', content: e.message || 'An error occurred' })}\n\n`,
        ),
      );
    } finally {
      await writer.close();
...
export async function GET(request: Request) {
  try {
    ...
    const suggestionLength =
      (url.searchParams.get('suggestionLength') as
        | 'short'
        | 'medium'
        | 'long'
        | null) || 'medium';
    const customInstructions =
      url.searchParams.get('customInstructions') || null;
    const writingStyleSummary =
      url.searchParams.get('writingStyleSummary') || null;
...
Instruction: "${description}"`
  ...
  const lengthMap = {
    short: 'concise',
    medium: 'a moderate amount of detail',
    long: 'comprehensively',
  };
...
Only output the resulting text, with no preamble or explanation.`;
  ...
  console.log(
    'Starting stream text generation with content length:',
    contentToModify.length,
    'and options:',
    { suggestionLength, customInstructions },
  );
  ...
  for await (const delta of fullStream) {
    ...
    if (type === 'text-delta') {
      ...
      if (chunkCount % 10 === 0) {
        console.log(
          `Stream progress: ${draftContent.length} characters processed (${chunkCount} chunks)`,
        );
      }
  ...
  console.log(
    `Stream complete: Generated ${draftContent.length} characters in ${chunkCount} chunks`,
  );
}

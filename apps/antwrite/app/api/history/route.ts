    ...
    if (uniqueDocIds.length > 0) {
      try {
        ...
        documents.forEach((doc) => {
          if (doc) {
            // Ensure document is not null/undefined
    ...
    const processedChats = chats.map((chat) => {
      ...
      return {
        ...
        document_context: {
          ...
          activeTitle: context.active
            ? documentTitles[context.active] || null
            : null,
          mentioned: context.mentioned || [],
          mentionedTitles: (context.mentioned || []).map(
            (id: string) => documentTitles[id] || null,
          ),
        },
      };
    });

...
export interface ArtifactActionContext<M = any> {
  content: string;
  handleVersionChange: (
    type: 'next' | 'prev' | 'toggle' | 'latest' | 'new',
  ) => void;
  currentVersionIndex: number;
  isCurrentVersion: boolean;
  mode: 'edit' | 'diff';
...
export function PureArtifact({
  ...
  const { renameDocument, isRenamingDocument, createDocument } =
    useDocumentUtils();
  ...
  useEffect(() => {
    ...
    if (
      ...
      (mostRecentDocument.content &&
        mostRecentDocument.content !== artifact.content)
    ) {
  ...
  const saveContent = useCallback(
    async (content: string, isDebounced = false) => {
      ...
      try {
        ...
        console.log(
          '[Artifact] Initiating save for document:',
          artifact.documentId,
        );
  ...
  const getSaveStatusDisplay = () => {
    if (saveState === 'saving') {
      return (
        <>
          <svg
            className="animate-spin size-3 text-muted-foreground"
            viewBox="0 0 24 24"
          >
            <circle
    ...
    return document ? (
      `Last saved ${formatDistance(new Date(document.createdAt), new Date(), {
        addSuffix: true,
      })}`
    ) : (

...
export function PublishSettingsMenu({
  ...
  const url =
    typeof window !== 'undefined'
      ? `${window.location.origin}/${sanitizedUsername}/${slug}`
      : `/${sanitizedUsername}/${slug}`;
    ...
    async (name: string, signal: AbortSignal) => {
      ...
      try {
        const res = await fetch(
          `/api/user?username=${encodeURIComponent(name.trim().replace(/^@/, ''))}`,
          { signal },
        );
        if (res.ok) {
  ...
  const handleToggle = useCallback(() => {
    ...
    const styleObj = document.style as any;
    const { h, s } =
      textColor !== undefined ? hexToHSL(textColor) : { h: 0, s: 0 };
    const snapshot = {
      id: document.id,
      ...
      style: {
        ...
        font,
        textColorLight:
          textColor !== undefined ? `hsl(${h}, ${s}%, 90%)` : undefined,
        textColorDark:
          textColor !== undefined ? `hsl(${h}, ${s}%, 20%)` : undefined,
      },
      slug,
    };
                    ...
                    <div className="flex items-center border-l border-input bg-background">
                      ...
                      {hasUsername && (
                        <Button
                          size="icon"
                          variant="ghost"
                          className="size-8 rounded-sm hover:bg-accent/50"
                          onClick={() => {
                            setHasUsername(false);
                            setUsernameCheck({
                              checking: false,

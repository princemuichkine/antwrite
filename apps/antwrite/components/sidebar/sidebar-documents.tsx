...
export function SidebarDocuments({
  ...
  const isGuestUser = user?.email?.includes('guest') || !user;
  const [isDocumentsExpanded, setIsDocumentsExpanded] = useState(!isGuestUser);
    ...
    async (documentId: string) => {
      ...
      try {
        ...
        setArtifact((curr: any) => {
          const newTitle = selectedDocData?.title || '';
  ...
  if (isLoading && documents.length === 0) {
    return (
      <SidebarGroup>
        <div className="px-0 py-1 text-xs text-sidebar-foreground/50"></div>
        <SidebarGroupContent>
              ...
              !searchTerm.trim() && (
                <div className="flex items-center justify-end px-0 py-1 mb-2">
                  <Button
                    variant="outline"
                    size="sm"
                    className="h-6 text-xs px-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-900/40 hover:text-green-800 dark:hover:text-green-200 border border-green-200 dark:border-green-800 transition-colors duration-200"
                    onClick={(e) => {
                      e.stopPropagation();
                      createNewDocument();
                    }}
                  ...
                  >
                    {isCreatingDocument ? (
                      ...
                      <FilePlusIcon className="size-3 text-green-700 dark:text-green-300" />
                    )}
                  </Button>
                </div>
              )}
            ...
            {filteredDocuments.length > 0 && (
              <div className="flex flex-wrap items-center justify-start px-0 py-1 mb-2 gap-2">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-6 text-xs px-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 hover:bg-green-100 dark:hover:bg-green-900/40 hover:text-green-800 dark:hover:text-green-200 border border-green-200 dark:border-green-800 transition-colors duration-200 ml-auto"
                  onClick={(e) => {
                    e.stopPropagation();
                    createNewDocument();
                  }}

...
function PureChatHeader({
  ...
  const { data: history, isLoading: isHistoryLoading } = useSWR<
    Array<Chat & { document_context?: any }>
  >('/api/history', fetcher, {
    fallbackData: [],
  ...
  const recentChats = history && history.length > 0 ? history.slice(0, 5) : [];
                      ...
                      {chat.document_context.mentioned &&
                        chat.document_context.mentioned.length > 0 && (
                          <div className="flex items-start gap-1.5 text-muted-foreground">
                            <span className="font-medium text-foreground/80 flex-shrink-0 pt-px">
                              Mentioned:
                            </span>

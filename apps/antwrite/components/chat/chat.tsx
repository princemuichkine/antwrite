...
export function Chat({
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      'reset-chat-input',
      handleResetChatInput as EventListener,
    );
  ...
  useEffect(() => {
    const loadHistory = async (idToLoad: string) => {
      console.log(
        `[Chat useEffect] Starting load for explicitly requested chatId: ${idToLoad}`,
      );
      ...
      try {
        ...
        if (!chatResponse.ok) {
          ...
          console.error(
            `[Chat useEffect] Fetch failed for ${idToLoad}. Status: ${chatResponse.status}. Body: ${errorText}`,
          );
        ...
        if (!chatData || !chatData.messages) {
          console.error(
            `[Chat useEffect] Invalid chat data received for ${idToLoad}. Data:`,
            chatData,
          );
  ...
  useEffect(() => {
    const handleLoadChatEvent = (event: CustomEvent<{ chatId: string }>) => {
      ...
      console.log(
        `[Chat EventListener] Received load-chat event for ${detail.chatId}. Current state chatId: ${chatId}. Setting new chatId.`,
      );
    ...
    window.addEventListener(
      'load-chat',
      handleLoadChatEvent as unknown as EventListener,
    );
  ...
  useEffect(() => {
    const handleChatIdChanged = (
      event: CustomEvent<{ oldChatId: string; newChatId: string }>,
    ) => {
      const { oldChatId, newChatId } = event.detail;

      if (oldChatId === chatId) {
        console.log(
          `[Chat] Changing chat ID from ${oldChatId} to ${newChatId}`,
        );
      }
    };
    ...
    window.addEventListener(
      'chat-id-changed',
      handleChatIdChanged as unknown as EventListener,
    );

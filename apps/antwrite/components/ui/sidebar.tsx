...
function Sidebar({
  ...
  // Desktop logic: left sidebar always visible (minimized when collapsed), right sidebar fully hidden when closed
  if (side === 'left') {
    return (
      <SidebarInnerContext.Provider value={side}>
        <div className="group/sidebar-wrapper flex h-full">
          <div
            className={cn(
          ...
          >
            <div
              data-sidebar="sidebar"
              className="flex h-full w-full flex-col bg-sidebar border-r group-data-[variant=floating]:rounded-sm group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
            >
              {children}
            </div>
          </div>
  ...
  if (side === 'right' && (state as string) !== 'expanded') {
    return null;
  }

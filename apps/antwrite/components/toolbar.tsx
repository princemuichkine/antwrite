  ...
  return (
    <Tooltip open={isHovered && !isAnimating}>
      ...
      <TooltipContent
        side="left"
        sideOffset={16}
        className="bg-foreground text-background rounded-sm p-3 px-4"
      >
        {description}
      </TooltipContent>
    </Tooltip>
      ...
      <TooltipProvider>
        <Tooltip open={!isAnimating}>
          <TooltipTrigger asChild>
            <motion.div
              className={cx(
                'absolute bg-background p-3 border rounded-sm flex flex-row items-center',
                {
          ...
          <TooltipContent
            side="left"
            sideOffset={16}
            className="bg-foreground text-background text-sm rounded-sm p-3 px-4"
          >
            {LEVELS[currentLevel]}
          </TooltipContent>
        </Tooltip>
...
const PureToolbar = ({
  ...
  if (!artifactDefinition) {
    console.error(
      `Artifact definition not found for kind: ${safeArtifactKind}`,
    );
    ...
    if (!textDefinition) {
      ...
      throw new Error(
        'Text artifact definition not found! This is a critical error.',
      );
    }
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <motion.div
        className="cursor-pointer absolute right-6 bottom-6 p-1.5 border rounded-sm shadow-lg bg-background flex flex-col justify-end"

...
const SuggestionOverlayContext =
  createContext<SuggestionOverlayContextType | null>(null);
  ...
  const handleAcceptSuggestion = useCallback(
    (suggestion: string) => {
      ...
      if (selectedText && selectedText.trim() !== '' && selectionRange) {
        console.log(
          `[Provider] Dispatching apply-suggestion event for range [${selectionRange.from}, ${selectionRange.to}]`,
        );
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
        ...
        if (activeEditorView?.state) {
          ...
          if (!empty) {
            ...
            console.log(
              `[Provider] Opening overlay via Cmd+K from editor state. Range: [${from}, ${to}]`,
            );
...
export function useSuggestionOverlay() {
  ...
  if (!context) {
    throw new Error(
      'useSuggestionOverlay must be used within a SuggestionOverlayProvider',
    );
  }
